...
- Beat %: color scale from red (0) through yellow (60) to green (100) — values stored as raw 0-100
- Allocation columns: light yellow highlight when non-empty
- Geo columns: light green highlight when non-empty

**Percentage handling (v5):** All percentage columns (Returns, Alpha, Beat %, Allocations, Geo)
store raw percentage numbers (e.g., 12.34 means 12.34%) with plain number format ("0.00"),
not Excel % format. Column headers include "(%) " suffix to indicate units. This simplifies
formula references and avoids confusion with Excel's divide-by-100 percentage format.
...
**Formula-driven (v5):** The Summary sheet uses live Excel formulas referencing the Master sheet
instead of Python-computed hardcoded values:
- Median statistics use `MEDIAN(FILTER(Master!col, Master!type=X))` formulas
- Top-N fund tables use `SORTBY(FILTER(...), FILTER(...), -1)` with `INDEX()` for each column
- This means the Summary auto-updates if Master data is manually edited
