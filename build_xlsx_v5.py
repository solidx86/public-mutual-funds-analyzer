...
# ── Data rows ────────────────────────────────────────────────────────────────
data_start = 4
text_cols = {1, 2, 3, 4, 5, 6, 8, 10, 11, 13, 63, 65, 66, 67}
...
def rank_f(dcol, sort_col, rank, fund_type, descending=True):
    """Returns SORTBY+FILTER+INDEX formula string."""
    cond = base_cond(fund_type)
    # Add condition to exclude blanks in sort column
    cond_no_blank = f'({cond})*({MASTER_REF}{sort_col}${data_start}:{sort_col}${last_row}<>"")'
    sort_order = -1 if descending else 1
    return f'=IFERROR(INDEX(SORTBY(FILTER({MASTER_REF}{dcol}${data_start}:{dcol}${last_row},{cond_no_blank}),FILTER({MASTER_REF}{sort_col}${data_start}:{sort_col}${last_row},{cond_no_blank}),{sort_order}),{rank},1),"")'

def write_top5(ws, r, title, headers, fund_type, sort_col, col_specs, color, nfmts=None):
    """Write a full Top-5 table using formulas.
    col_specs: list of (col_letter, is_sort) tuples
    ...
    for rank_num in range(1, 6):
        formulas = []
        for col_letter, is_sort in col_specs:
            if col_letter == "":
                formulas.append("")  # empty filler column
            else:
                f = rank_f(col_letter, sort_col, rank_num, fund_type)
                formulas.append(f)
...
def build_fund_type_section(ws, r, fund_type, color, fund_type_label=None):
    ...
    Uses formula-driven approach with MEDIAN(FILTER) and SORTBY/INDEX.
    """
