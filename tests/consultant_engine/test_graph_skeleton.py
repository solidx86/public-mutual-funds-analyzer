...
def test_graph_runs_end_to_end_with_stubs(tiny_fundmaster):
    app = build_graph(MemorySaver())
    cfg = {"configurable": {"thread_id": "t1"}}
    # no_review=True skips the interrupt so a single invoke runs to emit
    out = app.invoke(
        {"thread_id": "t1", "no_review": True, "fundmaster_path": tiny_fundmaster,
...
def test_interrupt_pauses_then_resumes(tiny_fundmaster):
    app = build_graph(MemorySaver())
    cfg = {"configurable": {"thread_id": "t2"}}
    out = app.invoke({"thread_id": "t2", "client_profile": {"risk_level": "Moderate"},
                      "fundmaster_path": tiny_fundmaster}, cfg)
