...
# --fundmaster optional: defaults to the newest FundMaster workbook on disk
# → pauses for review; run --resume <thread_id> to finalise, or --no-review to auto-approve
```

Sample profiles (one per risk level) ship in [`data/profiles/`](data/profiles/); `python -m consultant_engine --help` documents every flag and profile field.
