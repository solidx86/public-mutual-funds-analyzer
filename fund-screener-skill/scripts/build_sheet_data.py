...
CACHE_DIR = os.path.join(WORK_DIR, "data", "cache")
REF_DIR   = os.path.join(WORK_DIR, "data", "reference")
# RISK_LEVEL_FILE: leave as "" to auto-detect from data/reference/.
...
with open(os.path.join(CACHE_DIR, "mfr_results.json")) as f:
    data = json.load(f)

all_funds_raw = data['all_funds']
qualified_raw = data['qualified']

# ── Auto-detect risk file from data/reference/ ───────────────────────────────
if not RISK_LEVEL_FILE:
    candidate = os.path.join(REF_DIR, "funds_risk_level.xlsx")
    if os.path.exists(candidate):
        RISK_LEVEL_FILE = candidate
        print(f"Auto-detected risk level file: {RISK_LEVEL_FILE}")
    else:
        print(f"WARNING: funds_risk_level.xlsx not found in {REF_DIR}")
...
csv_path = os.path.join(CACHE_DIR, "master_funds.csv")
