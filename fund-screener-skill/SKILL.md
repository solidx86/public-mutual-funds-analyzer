...
**Data layout:** cache artifacts (`mfr_results.json`, `ath_results.json`, `fund_code_map.json`,
`master_funds.csv`) are read/written under `data/cache/`; reference data
(`funds_risk_level.xlsx`, `epf_qualified_funds.csv`) lives under `data/reference/`. Filenames
below name the artifact; the scripts resolve the `data/` paths automatically.

---

## Step 1: Extract MFR PDFs → mfr_results.json

MFR files live under `unit-trust/`. Named like:
```
...
**Before running:** Confirm the MFR PDFs are in `unit-trust/`. If the user says "new
...
- Cause: New MFR series added, or PDF not yet placed in `unit-trust/`
...
|---------|------|------|---------|
| 1.6 | 2026-06-15 | Refactor | Move pipeline data out of repo root into `data/cache/` (mfr/ath/fund_code_map/master_funds) and `data/reference/` (funds_risk_level.xlsx, epf_qualified_funds.csv); scripts and tests updated to the new layout |
